...
internal object BoardGenerationCoordinator {
    ...
    private suspend fun runGenerationBody(boardId: String) {
        ...
        val failedRecorder: suspend () -> Unit = { recordFailedRequest(boardId) }
        ...
        val textGenerator = LlmTextGenerator(
            chat = LlmSettings.textChat(usageRecorder, quotaRecorder, retryRecorder, failedRecorder),
        ...
        val balance = cachedBalance
            ?: llmBalanceGenerator(usageRecorder, quotaRecorder, retryRecorder, failedRecorder)
    ...
    private suspend fun recordFailedRequest(boardId: String) {
        boardMutex(boardId).withLock {
            val board = Storage.getBoardOrNull(boardId) ?: return
            val progress = board.generationProgress
            Storage.updateBoard(
                board.copy(
                    generationProgress = progress.copy(
                        failedRequestCount = progress.failedRequestCount + 1,
                    )
                )
            )
        }
            ...
            Storage.updateBoard(
                board.transform().copy(
                    generationProgress = BoardGenerationProgress(
                        ...
                        failedRequestCount = previousProgress.failedRequestCount,
