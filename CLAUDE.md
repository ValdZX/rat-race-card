...
- `server/src/main/resources/application.conf` binds `0.0.0.0` and honours `${?PORT}` / `${?HOST}` — required by any container host. The `main()` in `Application.kt` is unused by the packaged app (`mainClass` is `EngineMain`, which reads this file), so **anything that must run in production belongs in `Application.module()`**, not in `main()` — that is where `startBackgroundJobs()` (inactivity sweep + stuck-roll watchdog) is wired.
- Server event buses (`getGlobalEventBus`, `RaceRatServiceImpl.eventBus`, `checkStatusFlow`) must stay buffered with `BufferOverflow.DROP_OLDEST`. An unbuffered `MutableSharedFlow` deadlocks the whole board: `handleGlobalEvent` emits back into the bus it is collecting, so a rendezvous emit waits on itself forever. For the same reason, never `emit` while holding `boardMutex`/`playerMutex`.
- The server keeps all game state in-process (mutexes, `Storage` caches, event buses). It **must run as exactly one replica** — two instances do not share events and will overwrite each other's boards.
